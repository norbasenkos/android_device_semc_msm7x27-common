...
void load_volume_table() {
    ...
    while (fgets(buffer, sizeof(buffer)-1, fstab)) {
        for (i = 0; buffer[i] && isspace(buffer[i]); ++i);
        if (buffer[i] == '\0' || buffer[i] == '#' || buffer[i] == 'd') continue;

        char* original = strdup(buffer);
        char* mtddevice = strtok(buffer+i, ": ");
        char* size = strtok(NULL, " ");
        char* erasesize = strtok(NULL, " ");
        char* mount_point = strtok(NULL, " ");
        
        char* device = strdup(mount_point+1);
        ...
        if (mount_point && fs_type && device) {
            ...
            char tmp[PATH_MAX];
            sprintf(tmp, "/%s", mount_point+1);
            tmp[strlen(tmp)-2] = '\0';
            
            device_volumes[num_volumes].mount_point = strdup(tmp);
            device_volumes[num_volumes].fs_type = strdup(fs_type);
            //sprintf(tmp, "/dev/block/mtdblock%s", replace_str(device, "mtd", ""));
            device_volumes[num_volumes].device = strdup(device);
            device_volumes[num_volumes].length = 0;
            device_volumes[num_volumes].fs_options = NULL;
