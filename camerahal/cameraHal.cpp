...
{
   ALOGE("CameraHAL_NotifyCb: msg_type:%d ext1:%d ext2:%d user:%p\n",
                         ...
                         int x, int y, int w, int h)
{
    ...
    ALOGE("CameraHAL_CopyBuffers_Hw: srcFD:%d destFD:%d srcOffset:%#x"
                            ...
                            int32_t previewWidth, int32_t previewHeight)
{
   if (mWindow != NULL && getMemory != NULL) {
      ...
      ALOGE("CameraHAL_HandlePreviewData: previewWidth:%d previewHeight:%d "
      ...
      if (retVal == NO_ERROR) {
         ...
         ALOGE("CameraHAL_HandlePreviewData: dequeueing buffer\n");
            ...
            if (retVal == NO_ERROR) {
               ...
               if (!CameraHAL_CopyBuffers_Hw(mHeap->getHeapID(), privHandle->fd,
                  ...
                  mapper.lock(*bufHandle, GRALLOC_USAGE_SW_READ_OFTEN, bounds,
                              &bits);
                  ALOGE("CameraHAL_HPD: w:%d h:%d bits:%p",
               ...
               ALOGE("CameraHAL_HandlePreviewData: enqueued buffer");
            } else {
                        ...
                        void *user)
{
   ...
   ALOGE("CameraHAL_GenClientData: offset:%#x size:%#x base:%p\n",
        (unsigned)offset, size, mHeap != NULL ? mHeap->base() : 0);
...
CameraHAL_DataCb(int32_t msg_type, const android::sp<android::IMemory>& dataPtr,
                 void *user)
{
   ALOGE("CameraHAL_DataCb: msg_type:%d user:%p\n", msg_type, user);
   ...
   if (origData_cb != NULL && origCamReqMemory != NULL) {
      ...
      if (clientData != NULL) {
         ALOGE("CameraHAL_DataCb: Posting data to client\n");
...
{
   ALOGE("CameraHAL_DataTSCb: timestamp:%lld msg_type:%d user:%p\n",
   ...
   if (origDataTS_cb != NULL && origCamReqMemory != NULL) {
      camera_memory_t *clientData = CameraHAL_GenClientData(dataPtr,
                                       origCamReqMemory, user);
      if (clientData != NULL) {
         ALOGE("CameraHAL_DataTSCb: Posting data to client timestamp:%lld\n",
              systemTime());
...
{
   ALOGE("CameraHAL_GetNum_Cameras: 1\n");
   return 1;
}
...
{
   ALOGE("qcamera_set_preview_window : Window :%p\n", window);
   if (device == NULL) {
...
{
   ALOGE("qcamera_set_callbacks: notify_cb: %p, data_cb: %p "
...
{
   ALOGE("qcamera_enable_msg_type: msg_type:%#x\n", msg_type);
   if (msg_type == 0xfff) {
      msg_type = 0x1ff;
   } else {
...
{
   ALOGE("qcamera_disable_msg_type: msg_type:%#x\n", msg_type);
   if (msg_type == 0xfff) {
      msg_type = 0x1ff;
   }
...
{
   ALOGE("qcamera_msg_type_enabled: msg_type:%d\n", msg_type);
...
{
   ALOGE("qcamera_start_preview: Enabling CAMERA_MSG_PREVIEW_FRAME\n");

   /* TODO: Remove hack. */
   ALOGE("qcamera_start_preview: Preview enabled:%d msg enabled:%d\n",
...
{
   ALOGE("qcamera_stop_preview: msgenabled:%d\n",
        qCamera->msgTypeEnabled(CAMERA_MSG_PREVIEW_FRAME));
...
{
   ALOGE("qcamera_store_meta_data_in_buffers:\n");
   return NO_ERROR;
}
...
{
   ALOGE("qcamera_recording_enabled:\n");
                                ...
                                const void *opaque)
{
    ...
    */
   ALOGE("qcamera_release_recording_frame: opaque:%p\n", opaque);
}

int
qcamera_auto_focus(struct camera_device * device)
{
   ALOGE("qcamera_auto_focus:\n");
   qCamera->autoFocus();
   return NO_ERROR;
}
...
{
   ALOGE("qcamera_cancel_auto_focus:\n");
...
{
   ALOGE("qcamera_set_parameters: %s\n", params);
...
{
   char *rc = NULL;
   ALOGE("qcamera_get_parameters\n");
   camSettings = qCamera->getParameters();
   ALOGE("qcamera_get_parameters: after calling qCamera->getParameters()\n");
   CameraHAL_FixupParams(camSettings);
   g_str = camSettings.flatten();
   rc = strdup((char *)g_str.string());
   ALOGE("camera_get_parameters: returning rc:%p :%s\n",
        rc, (rc != NULL) ? rc : "EMPTY STRING");
   return rc;
}
...
{
   ALOGE("qcamera_put_parameters: params:%p %s", params, params);
   free(params);
}
...
{
   ALOGE("qcamera_send_command: cmd:%d arg0:%d arg1:%d\n",
        cmd, arg0, arg1);
