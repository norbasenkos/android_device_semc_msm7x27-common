...
static int wired_headphone = 0;
static int wired_headset = 0;
...
//static uint32_t SND_DEVICE_HEADSET_AND_SPEAKER=26; // really FORCE_SPEAKER     SND_DEVICE_FARFIELD_HEADSET in mine
...
status_t AudioHardware::setParameters(const String8& keyValuePairs)
{
    ...
    if (param.get(key, value) == NO_ERROR) {
       if (value == "on") {
          ALOGI("fm_radio_active true...");
          setFmOnOff(true);
       }
       else if (value == "off") {
          ALOGI("fm_radio_active false...");
          setFmOnOff(false);
       }
    }
    ...
    if (param.get(key, value) == NO_ERROR) {
       if (value == "on") {
          ALOGI("fm_radio_speaker true...");
          setFmSpeakerOnOff(true);
       }
       else if (value == "off") {
          ALOGI("fm_radio_speaker false...");
          setFmSpeakerOnOff(false);
       }
    }

//wired_accessory_class=headphone
//wired_accessory_class=headset
    const char WIRED_ACCESSORY_CLASS[] = "wired_accessory_class";
    key = String8(WIRED_ACCESSORY_CLASS);
    if (param.get(key, value) == NO_ERROR) {
       if (value == "headphone") { //two lines headphones
          ALOGV("headphone ON!");
          wired_headphone = 1;
          wired_headset = 0;
       }
       else if (value == "headset") { //three lines headphones
          ALOGV("headset ON!");
          wired_headphone = 0;
          wired_headset = 1;
       }
       else if (value == "none") { //no wired accessory is connected
          ALOGV("headphone/headset OFF!");
          wired_headphone = 0;
          wired_headset = 0;
       }
    }
...
status_t AudioHardware::setFmOnOff(bool onoff)
{
    ...
    if (onoff) {
        ALOGI("Setting fm radio to true");
        wired_headphone = 0;
        wired_headset = 0;
        mFmRadioEnabled = true;
    } else {
        ALOGI("Setting fm radio to false");
...
status_t AudioHardware::doRouting(AudioStreamInMSM72xx *input)
{
    ...
    ALOGV("wired_headphone is %d wired_headset is %d", wired_headphone, wired_headset);
    if (wired_headphone == 1) {
        new_snd_device = SND_DEVICE_HEADSET;
    }
    else if (wired_headset == 1) {
